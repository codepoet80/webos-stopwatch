...
TimerAssistant.prototype.activate = function(event) {
    ...
    if (appModel.AlarmLaunch) {
        Mojo.Log.info("*** timer scene activated because of alarm");
        ...
        if (appModel.AppSettingsCurrent["TimerRunning"] == true && appModel.AppSettingsCurrent["TimerEndTime"] != "null") {
            Mojo.Log.info("*** timer scene is attempting to re-hydrate a running timer");
            var remainAlarmDifference = Date.parse(appModel.AppSettingsCurrent["TimerEndTime"]) - new Date();
            Mojo.Log.info("*** previously running timer delta from now: " + remainAlarmDifference);
            if (remainAlarmDifference > 0) {
                ...
                Mojo.Log.info("*** resuming remaining alarm timer: " + remainAlarmDifference);
                ...
                Mojo.Log.info("*** timer scene is dicarding a previously running timer that has expired");
                appModel.AppSettingsCurrent["TimerEndTime"] = "null";
                appModel.AppSettingsCurrent["TimerRunning"] = false;
                this.setUIForReset();
            }
        } else {
            Mojo.Log.info("*** timer scene has no active timers");
            this.setUIForReset();
        }
    }
...
TimerAssistant.prototype.startLocalTimer = function(Hours, Mins, Seconds) {
    ...
    Mojo.Log.info("Timer started at: " + timerValue);
    Mojo.Log.info("Timer will end in: " + timerDuration + "ms");
    timerValue.setMilliseconds(timerValue.getMilliseconds() + timerDuration);
    Mojo.Log.info("Timer end time is: " + timerValue.toLocaleString());
...
TimerAssistant.prototype.incrementTimer = function(showTimerValue) {
    ...
    if (!showTimerValue) {
        ...
        Mojo.Log.info("*** previously running timer delta from now: " + showTimerValue);
    } else {
...
TimerAssistant.prototype.timerDone = function() {
    Mojo.Log.info("timer done called");
    ...
    try {
        Mojo.Log.info("timer done is asking UI to reset");
        this.setUIForReset();
    } catch (error) {
